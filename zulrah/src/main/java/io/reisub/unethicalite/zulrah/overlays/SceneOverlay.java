...
public class SceneOverlay extends Overlay {
  ...
  private void renderZulrahPhaseTiles(Graphics2D graphics) {
    if (config.phaseDisplayType() != ZulrahConfig.DisplayType.OFF
        && config.phaseDisplayType() != ZulrahConfig.DisplayType.OVERLAY) {
      SetMultimap<ZulrahLocation, MutablePair<String, ZulrahNpc>> zulrahLocationsGrouped =
          LinkedHashMultimap.create();
      plugin.getZulrahData().forEach((data) -> {
        switch (config.phaseDisplayMode()) {
          case CURRENT:
            data.getCurrentZulrahNpc().ifPresent((npc) -> {
              zulrahLocationsGrouped.put(npc.getZulrahLocation(),
                  new MutablePair<>("Current", npc));
            });
            break;
          case NEXT:
            data.getNextZulrahNpc().ifPresent((npc) -> {
              zulrahLocationsGrouped.put(npc.getZulrahLocation(),
                  new MutablePair<>(getZulrahNextString(), npc));
            });
            break;
          case BOTH:
            data.getCurrentZulrahNpc().ifPresent((npc) -> {
          ...
          default:
            throw new IllegalStateException(
                "[SceneOverlay] Invalid 'phaseDisplayMode' config state");
        }
      ...
      while (location.hasNext()) {
        ...
        for (Iterator groupedLocation = zulrahLocationsGrouped.get(zulrahLocation).iterator();
            groupedLocation.hasNext(); offset += graphics.getFontMetrics().getHeight()) {
  ...
  private void renderStandAndStallTiles(Graphics2D graphics) {
    if (config.standLocations() || config.stallLocations()) {
        ...
        if (config.standLocations()) {
          if (data.standLocationsMatch()) {
            data.getCurrentDynamicStandLocation().ifPresent((loc) -> {
              standLocationsGrouped.put(loc,
                  new MutablePair("Stand / Next", config.standAndNextTileColor()));
            });
          } else {
            data.getCurrentDynamicStandLocation().ifPresent((loc) -> {
      ...
      while (location.hasNext()) {
        ...
        for (Iterator locationGrouped = standLocationsGrouped.get(standLocation).iterator();
            locationGrouped.hasNext(); offset += graphics.getFontMetrics().getHeight()) {
          Pair pair = (Pair) locationGrouped.next();
          drawTile(graphics, standLocation.toLocalPoint(), (String) pair.getLeft(),
              (Color) pair.getRight(), offset);
        }
      }
