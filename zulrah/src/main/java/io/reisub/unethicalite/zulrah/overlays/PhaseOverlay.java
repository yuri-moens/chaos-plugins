...
public class PhaseOverlay
  ...
  public Dimension render(Graphics2D graphics) {
    if (config.phaseDisplayType() == ZulrahConfig.DisplayType.OFF
        || config.phaseDisplayType() == ZulrahConfig.DisplayType.TILE
        || plugin.getZulrahNpc() == null || plugin.getZulrahNpc().isDead()) {
      return null;
    }
    LinkedHashMultimap<ZulrahType, MutablePair<String, Boolean>> zulrahPhases =
        LinkedHashMultimap.create();
    plugin.getZulrahData().forEach(data -> {
      switch (config.phaseDisplayMode()) {
        case CURRENT: {
          data.getCurrentPhase().ifPresent(phase -> zulrahPhases.put(phase.getZulrahNpc().getType(),
              new MutablePair<String, Boolean>("Current", false)));
          break;
        }
        case NEXT: {
          data.getNextPhase().ifPresent(phase -> zulrahPhases.put(phase.getZulrahNpc().getType(),
              new MutablePair<String, Boolean>(getNextString(), true)));
          break;
        }
        case BOTH: {
          data.getCurrentPhase().ifPresent(phase -> zulrahPhases.put(phase.getZulrahNpc().getType(),
    ...
    List<InfoBoxComponent> components = zulrahPhases.entries().stream().map(this::infoBoxComponent)
  ...
  private InfoBoxComponent infoBoxComponent(
      Map.Entry<ZulrahType, MutablePair<String, Boolean>> entry) {
