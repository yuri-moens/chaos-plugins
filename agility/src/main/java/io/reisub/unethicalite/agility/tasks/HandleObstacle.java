...
public class HandleObstacle extends Task {
    ...
    public boolean validate() {
        if (config.highAlch() && Movement.getDestination() != null && Players.getLocal().distanceTo(Movement.getDestination()) > 5) {
            return false;
        }
    ...
    public void execute() {
        ...
        if (config.courseSelection() == Course.PRIFDDINAS) {
            ...
            if (portal != null && portal.getClickbox() != null && config.courseSelection().isReachable(portal)) {
                portal.interact(0);
                ready = false;
                return;
            }
